import { SystemState, User, UserRole, Task, TaskPriority, AIModule, SystemStatus, Simulation, SimulationStatus, Block, LogEntry, Device, FederationNetwork, UpgradeProposal, WebSocketMessage, FederationNode, SimulationProposal, AITaskSuggestion, HandshakeInvitation, HandshakeStatus } from '../types';
...
// FIX: Added 'as const' to ensure TypeScript infers the correct literal type for the log level.
    ...
    case 'INITIATE_NUN_HANDSHAKE': {
        const newInvitation: HandshakeInvitation = {
            id: `inv-${generateRandomHash(8)}`,
        ...
        return {
            ...state,
            federationControl: {
                ...state.federationControl,
                invitationTarget: '', // Clear the input field
                invitations: [newInvitation, ...state.federationControl.invitations],
            },
// FIX: Added 'as const' to ensure TypeScript infers the correct literal type for the log level.
    ...
    case 'WEBSOCKET_MESSAGE_RECEIVED': {
        ...
        switch (type) {
            ...
            case 'HEALTH_CHECK_UPDATE': {
                ...
                let updatedChecks = [...state.healthChecks];

                updates.forEach(update => {
                    const index = updatedChecks.findIndex(check => check.name === update.name);
                    if (index !== -1) {
                        updatedChecks[index] = { ...updatedChecks[index], status: update.status };
                    }
                });
...
// FIX: Added 'as const' to ensure TypeScript infers the correct literal type for the log level.
            ...
            case 'HANDSHAKE_STATUS_UPDATE': {
                return {
                    ...state,
                    federationControl: {
                        ...state.federationControl,
                        invitations: state.federationControl.invitations.map(inv =>
                            inv.id === payload.invitationId ? { ...inv, status: payload.status } : inv
                        )
                    }
                };
            }
            case 'VAULT_STATUS_UPDATE': {
                const isUsurped = payload.status === 'Usurped';
                return {
                    ...state,
                    nunVault: {
                        ...state.nunVault,
                        status: payload.status,
                        distribution: { // Also update distribution to match state
                            transfer: isUsurped ? 80 : 33.3,
                            atmospheric: isUsurped ? 10 : 33.3,
                            orbital: isUsurped ? 10 : 33.4,
                        },
                    },
                    logs: [
// FIX: Added 'as const' to ensure TypeScript infers the correct literal type for the log level.
                        { timestamp: now, level: 'TRACE' as const, message: `Remote event: NuN Vault status changed to ${payload.status}.` },
                        ...state.logs
                    ].slice(0, 100)
                };
            }
            case 'VAULT_SNAPSHOT_CREATED': {
                return {
                    ...state,
                    nunVault: {
                        ...state.nunVault,
                        lastSnapshotTimestamp: payload.timestamp
                    },
                    notifications: [
                        ...state.notifications,
                        { id: generateRandomHash(8), read: false, type: 'info', message: 'A new system snapshot was created in the NuN Vault.' }
                    ],
                    logs: [
// FIX: Added 'as const' to ensure TypeScript infers the correct literal type for the log level.
                        { timestamp: now, level: 'TRACE' as const, message: `Remote event: New NuN Vault snapshot created.` },
                        ...state.logs
                    ].slice(0, 100)
                };
            }
